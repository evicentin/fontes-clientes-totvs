...
While !SZIQRY->(EOF())
    ...
    nPos := Ascan(aMovimento, {|x|, x[1] == SZIQRY->ZH_CODPOST .and. x[2] == SZIQRY->ZH_LOCALID .and.;
        x[3] == SZIQRY->ZH_CC .and. x[4] == SZIQRY->ZI_PRODUTO})
    ...
    If nPos = 0
        ...
        Else
            aMovimento[nPos, 5] += SZIQRY->ZI_QUANT
        EndIf
	SZIQRY->(DbSkip())
End
SZIQRY->(DbCloseArea())
...
Local cProduto  := aCustos[1, 1] + aCustos[1, 3] + aCustos[1, 8] + aCustos[1, 9]
...
For nX := 1 to Len(aCustos)
    If cProduto <> aCustos[nX, 1] + aCustos[nX, 3] + aCustos[nX, 8] + aCustos[nX, 9] 
        cProduto := aCustos[nX, 1] + aCustos[nX, 3] + aCustos[nX, 8] + aCustos[nX, 9]
    ...
    nTotal += aCustos[nX, 14]
Next nX

// Imprime as últimas linhas do totalizador.
oExcel:AddRow("Custos","Locação",{;
    "",;
    "",;
    "",;
